    ...
    if (search) {
      ...
      list = list.filter(
        (a) =>
          ...
          a.title_km?.toLowerCase().includes(q)
      );
    }
    setItems(list);
    setLoading(false);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {locale === "km" ? "គ្រប់គ្រងសកម្មភាព" : "Activities Management"}
          </h1>
          <p className="text-gray-500 text-sm mt-1">{items.length} activities</p>
        </div>
        <Button asChild className="bg-school-blue-800 hover:bg-school-blue-900">
          <Link href={`/${locale}/admin/activities/new`}>
            <Plus className="w-4 h-4 mr-2" />
            {locale === "km" ? "បន្ថែម" : "New Activity"}
          </Link>
        </Button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <Input
            placeholder={locale === "km" ? "ស្វែងរក..." : "Search activities..."}
            className="pl-9"
            value={search}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            {locale === "km" ? "រកមិនឃើញ" : "No activities found"}
          </div>
        ) : (
                        ...
                        <div className="flex items-center gap-3">
                          {item.featured_image && (
                            <div className="w-10 h-8 rounded overflow-hidden shrink-0">
                              <Image src={item.featured_image} alt={title} width={40} height={32} className="object-cover w-full h-full" />
                            </div>
                          )}
                          <p className={`font-medium text-gray-900 truncate max-w-[220px] ${locale === "km" ? "font-khmer" : ""}`}>
                            {title}
                          </p>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <Button asChild variant="ghost" size="icon" className="h-8 w-8">
                            <Link href={`/${locale}/admin/activities/${item.id}`}>
