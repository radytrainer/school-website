...
import { Plus, Search, Edit, Trash2, Loader2, Download, FileText } from "lucide-react";
...
export default function AdminDownloadsPage() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    let query = supabase.from("downloads").select("*, category:download_categories(*)").order("created_at", { ascending: false });
    if (statusFilter !== "all") query = query.eq("status", statusFilter);
    const { data } = await query;
    let list = (data ?? []) as DownloadType[];
    if (search) {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    if (result.success) { toast.success("Download deleted"); fetchItems(); }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {locale === "km" ? "គ្រប់គ្រងឯកសារ" : "Downloads Management"}
          </h1>
          <p className="text-gray-500 text-sm mt-1">{items.length} files</p>
        </div>
        <Button asChild className="bg-school-blue-800 hover:bg-school-blue-900">
          <Link href={`/${locale}/admin/downloads/new`}>
            <Plus className="w-4 h-4 mr-2" />
            {locale === "km" ? "បន្ថែម" : "Add File"}
          </Link>
        </Button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <Input placeholder={locale === "km" ? "ស្វែងរក..." : "Search files..."} className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">{locale === "km" ? "រកមិនឃើញ" : "No files found"}</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-500 hidden lg:table-cell">Size</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-500 hidden lg:table-cell">Downloads</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item, i) => {
                  ...
                  const catName = item.category ? getLocalizedText(item.category.name_km, item.category.name_en, locale) : null;
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-400 uppercase">{item.file_type?.split("/").pop()}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell text-gray-500 text-xs">
                        <span className="flex items-center gap-1"><Download className="w-3 h-3" />{item.download_count ?? 0}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <Button asChild variant="ghost" size="icon" className="h-8 w-8">
                            <Link href={`/${locale}/admin/downloads/${item.id}`}><Edit className="w-4 h-4 text-blue-500" /></Link>
                          </Button>
